...
public class ExtractDataService {
	...
	public synchronized List<Map<String, Object>> buildData(ExtractRequestParams params)
	...
	private void getNcFile(ExtractRequestParams params) {
		...
		if (Constants.TIMEPOINT_A.equals(params.getTimePoint())
				&& Constants.SHOW_TYPE_CONCN.equals(params.getShowType())) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				try {
					...
					ncFile1 = nc1;
					logger.info("open nc file" + filePath + ", times = " + (System.currentTimeMillis() - openNcTimes)
							+ "ms");
			...
			try {
				...
				logger.info(
						"open nc file" + filePath1 + ", times = " + (System.currentTimeMillis() - openNcTimes) + "ms");
			} catch (IOException e) {
			...
			if (!Constants.CALCTYPE_SHOW.equals(params.getCalcType())) {
				...
				try {
					...
					logger.info("open nc file " + filePath2 + ", times = " + (System.currentTimeMillis() - openNcTimes)
							+ "ms");
				} catch (IOException e) {
	...
	public List<PointBean> buildPointList(ExtractRequestParams params, List<Map<String, Object>> res)
	...
	private double getValue(PointBean pb, ExtractRequestParams params) throws IOException, InvalidRangeException {
		...
		if (Constants.CALCTYPE_SHOW.equals(params.getCalcType())) {
			...
			if (Constants.TIMEPOINT_A.equals(params.getTimePoint())) {
				...
				if (value1 == Constants.OVERBORDER)
					return Constants.OVERBORDER;
				double value2 = interpolation.interpolation(variable2, p);
				if (value2 == Constants.OVERBORDER)
					return Constants.OVERBORDER;
				value = value2 - value1;
			}
