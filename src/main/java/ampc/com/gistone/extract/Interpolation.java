...
public class Interpolation {
	...
	public Interpolation(int hour, int layer, double xorig, double yorig, double xcell, double ycell, int row,
			int col) {
		this.hour = hour;
		this.layer = layer;
		this.xorig = xorig;
	...
	public double interpolation(Variable variable, Point2D point2D) throws IOException, InvalidRangeException {
		...
		if (subx < 0 || subx > col - 1 || suby < 0 || suby > row - 1 || x2 < 0 || x2 > col - 1 || y2 < 0
				|| y2 > row - 1) {
